...
```json
{
  ...
  "operating_model": ["Mid-size agency", "In-house production and post"],
  "studio_personality": ["Structured", "Process-driven", "Scalable"],
  "creative_dna": [],
  "visual_dna": [],
  "decision_maker": "Head of Production (Priya)",
  "buyer_personality": ["Process-driven", "Vendor-managed"],
  "human_angle": "UNKNOWN",
  ...
  "confidence": "Medium",
  "reasoning": "Operating model and decision-maker are directly stated. No visual signature is stated either, so creative_dna and visual_dna are correctly left empty rather than invented, and human_angle is UNKNOWN rather than a forced compliment."
}
```

This is good because it shows the honest alternative to the founder-led
example: `studio_personality` is "Structured, Process-driven, Scalable"
instead of "Independent, Hands-on" — the field is doing real work
distinguishing how you'd approach this lead differently. `creative_dna` and
`visual_dna` are empty arrays, not filler, because the evidence explicitly
says there's no consistent signature. `human_angle` is UNKNOWN rather than
invented. This lead is still worth contacting (`outreach_priority: Medium`)
— it's just a different kind of relationship than a boutique.
